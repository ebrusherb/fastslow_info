function p = payoffs_assortment(x,b,c,R,po,rhoinfo,pr,s,ps,rhoint)
...
% cost=s;
cost=s*(k1*(1-x(3))+k3*x(3));

p=zeros(3,1);
p(1)=b*X(1,1)+b*ps*(R-k1)*X(1,3)+b*k1*X(1,3)-c*R;
p(2)=b*X(2,1)+b*ps*(R-k2)*X(2,3);
p(3)=b*X(3,1)+b*ps*(R-k3)*X(3,3)-c*ps*(R-k1*X(3,1)-k2*X(3,2)-k3*X(3,3))-c*k1*X(3,1)+(b-c)*g*X(3,3)-cost;
