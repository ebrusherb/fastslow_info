...
%         cost=s;
        ...
        p3=b*X(3,1)+b*ps*(R-k3)*X(3,3)-c*ps*(R-k1*X(3,1)-k3*X(3,3))-c*k1*X(3,1)+(b-c)*g*X(3,3)-cost;
        z=p3-p1;
    end
...
%     x3=c*R/(b*k1); %find interior equilibrium 
    function z = fi(x1)
        ...
        k3=kbar(R,po3,pr);
%         cost=s;
        cost=s*(k1*x1+k2*(1-x3-x1)+k3*x3);
        g=gbar_assortment(R,po,rhoinfo,pr,x1,x3,ps,rhoint);
        X=interaction_matrix([x1 x2 x3],rhoint);
        p2=b*X(2,1)+b*ps*(R-k2)*X(2,3);
        p3=b*X(3,1)+b*ps*(R-k3)*X(3,3)-c*ps*(R-k1*X(3,1)-k2*X(3,2)-k3*X(3,3))-c*k1*X(3,1)+(b-c)*g*X(3,3)-cost;
        z=p3-p2;
    end
