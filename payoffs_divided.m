function v = payoffs_divided(x,b,c,R,po,poprime,rhoinfo,pr,prprime,s,ps,rhoint)
% 
...
% 
% P=[+b*g*X(3,3),-c*ps*(R-k1*X(3,1)-k2*X(3,2)-k3*X(3,3)),-c*k1*X(3,1),-c*g*X(3,3)];
% 
% %from residents to good, to strangers, to known cooperators, to good residents
...
v=[b*X(4,1)-b*X(3,1)
    b*ps*(R-k(1,4))*X(4,3)-b*ps*(R-k(1,3))*X(3,3)
    b*g(1,2)*X(4,3)-b*g(1,1)*X(3,3)
    b*ps*(R-k(2,4))*X(4,4)-b*ps*(R-k(2,3))*X(3,4)
    b*g(2,2)*X(4,4)-b*g(2,1)*X(3,4)
    -c*ps*(R-k(2,1)*X(4,1)-k(2,2)*X(4,2)-k(2,3)*X(4,3)-k(2,4)*X(4,4))-(-c*ps*(R-k(1,1)*X(3,1)-k(1,2)*X(3,2)-k(1,3)*X(3,3)-k(1,4)*X(3,4)))
    -c*k(2,1)*X(4,1)-(-c*k(1,1)*X(3,1))
    -c*g(2,1)*X(4,3)-(-c*g(1,1)*X(3,3))
    -c*g(2,2)*X(4,4)-(-c*g(1,2)*X(3,4))];